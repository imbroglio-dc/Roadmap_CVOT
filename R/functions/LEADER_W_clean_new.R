...
if (file.exists("data/combined_data_12-2021.csv")) {
    W_imputed <- read_csv("data/combined_data_12-2021.csv") %>%
        mutate(RETINSEV = factor(case_when(is.na(RETINSEV) ~ "NA",
                                           T ~ RETINSEV), ordered = T,
                                 levels = c("NA", "no retinopathy", "non-proliferative", "proliferative")),
               NYHACLAS = factor(case_when(is.na(NYHACLAS) ~ "NA",
    ...
    W_imputed <- dplyr::select_if(W_new, anyNA) %>%
    ...
    ## 25 imputation flags, 80 covariates, 15/80 are binning/cutoffs, + USUBJID and ARM - 107 columns total ----
    write_csv(W_imputed, "data/combined_data_12-2021.csv")
}
